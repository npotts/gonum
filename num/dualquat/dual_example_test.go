...
func raiseDual(p point) dualquat.DualQuat {
	return dualquat.DualQuat{
		Real: quat.Quat{Real: 1},
		Dual: raise(p),
	}
}
...
// vectors.
func transform(p point, by ...dualquat.DualQuat) point {
	if len(by) == 0 {
		return p
	}
...
func Example() {
	...
	rotate := dualquat.DualQuat{Real: axis}
