...
// DualQuat is a float64 precision dual quaternion.
...
func (d DualQuat) Format(fs fmt.State, c rune) {
...
func Add(x, y DualQuat) DualQuat {
	return DualQuat{
...
func Sub(x, y DualQuat) DualQuat {
	return DualQuat{
...
func Mul(x, y DualQuat) DualQuat {
	return DualQuat{
...
func Inv(d DualQuat) DualQuat {
	return DualQuat{
		Real: quat.Inv(d.Real),
		Dual: quat.Scale(-1, quat.Mul(d.Dual, quat.Mul(d.Real, d.Real))),
	}
}

// ConjDual returns the dual conjugate of d₁+d₂ϵ, d₁-d₂ϵ.
func ConjDual(d DualQuat) DualQuat {
	return DualQuat{
		Real: d.Real,
...
func ConjQuat(d DualQuat) DualQuat {
	return DualQuat{
...
func Scale(f float64, d DualQuat) DualQuat {
	return DualQuat{Real: quat.Scale(f, d.Real), Dual: quat.Scale(f, d.Dual)}
}

// Abs returns the absolute value of d.
// func Abs(d DualQuat) dual.Number {
// 	return Dual{
