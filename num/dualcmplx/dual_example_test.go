...
func raiseDual(p point) dualcmplx.DualCmplx {
	return dualcmplx.DualCmplx{
		Real: 1,
...
// vectors.
func transform(p point, by ...dualcmplx.DualCmplx) point {
	if len(by) == 0 {
		return p
	}
...
func Example() {
	...
	for i, p := range []point{
		...
		pp := transform(p,
			...
			// Rotate.
			dualcmplx.DualCmplx{Real: complex(math.Cos(math.Pi/2), math.Sin(math.Pi/2))},
		)
